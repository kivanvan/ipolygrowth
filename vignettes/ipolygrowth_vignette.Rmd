---
title: "Using polygrowth for bacterial growth estimation"
output: 
  rmarkdown::html_vignette:
    toc: true
vignette: >
  %\VignetteIndexEntry{Using polygrowth for bacterial growth estimation}
...
For more details about this data, see https://cran.r-project.org/web/packages/growthrates/vignettes/Introduction.html.  
...
The input data for`ipg_multisample()` is the same long format data used for `ipg_singlesample()` with the addition of the ID variable.  `epsilon` is specified as a vector of values, one for each sample, thus allowing different thresholds for each sample.
