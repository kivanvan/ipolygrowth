...
The `ipolygrowth` package is designed to estimate bacterial growth curve parameters using the 4th-degree polynomial model [(Cross et al. 2015)](https://pubmed.ncbi.nlm.nih.gov/26593732/). Growth curve parameters are estimated from 4th-degree polynomial models. The package is capable of taking either a single sample or multiple samples, with or without technical replicates. We refer to the biological replicate as the sample in our package and the vignette. Although `ipolygrowth` is designed for bacterial data, the use of this package may be expanded to any growth curve data.  
...
We will use the bacterial growth data from [growthrates package](https://CRAN.R-project.org/package=growthrates) for demonstration. This data contains 36 samples from 3 strains of bacteria and 12 levels of antibiotics tetracycline concentration. Each sample has 2 technical replicates. For more details about this data, see https://cran.r-project.org/web/packages/growthrates/vignettes/Introduction.html.  
...
The sample data is a ready-to-use example with multiple samples. For the single sample scenario, you will see an example in the below section.  
...
out.multi.r <- ipg_multisample(df.gr.r, "conc", "time", "value", threshold)
...
out.multi.f <- ipg_multisample(df.gr, c("strain", "conc"), "time", "value", 0.2/100)
